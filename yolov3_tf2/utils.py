...
def draw_outputs(img, outputs, class_names):
    ...
    # For aiming application (ANDREW AND JASONS CODE)
    w = img.width
    h = img.height
    ...
    for i in range(nums):
        if(classes[i] == 0 and objectness[i] >= 0.90): # For Aiming Application
            ...
            for t in np.linspace(0, 1, thickness):
                ...
                draw.line([aimpoint, center])
                draw.point(aimpoint, fill=tuple(color))
                #
